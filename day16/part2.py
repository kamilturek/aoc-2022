...
def solve(input):
    ...
    all_opened = (1 << len(costs)) - 1
    max_flow = 0
    for opened in range((all_opened + 1) // 2):
        opened_by_human = open_valve("AA", opened)
        opened_by_elephant = open_valve("AA", opened_by_human ^ all_opened)
        flow = dfs("AA", 26, opened_by_human) + dfs("AA", 26, opened_by_elephant)
